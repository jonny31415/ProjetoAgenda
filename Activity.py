...
class ActivityHandler:
    def __init__(self) -> None:
        ...
        self.cursor.execute(f"create table if not exists {self.table_name}\
                             (id integer primary key autoincrement, date datetime, remainder integer, message text)")
    
    def add_activity(self, act: Activity) -> None:
        self.cursor.execute(f"insert into {self.table_name} values (?,?,?,?)", (None, act.date, act.remainder, act.message))
        self.connection.commit()
    
    def edit_activity(self, id: int, act: Activity) -> None:
        self.cursor.execute(f"update {self.table_name} set date=?, remainder=?, message=? where id=?", (act.date, act.remainder, act.message, id))
        self.connection.commit() 
    ...
    def get_activities(self) -> list[tuple, Activity]:
        ...
        for row in self.cursor.execute(f"select id, date, remainder, message from {self.table_name}"):
